...
\lcode{kernel_displ_EVENTID} (if \lcode{USE_PATH_SPECIFIC_MODELS = .false.})\\
\lcode{kernel_displ_EVENTID_STATIONNAME} (if \lcode{USE_PATH_SPECIFIC_MODELS = .true.})
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
of following format (dependent on the forward method possibly extended by some file extension):\\
\lcode{kernel_gt_STATIONNAME} (if \lcode{USE_PATH_SPECIFIC_MODELS = .false.})\\
\lcode{kernel_gt_EVENTID_STATIONNAME} (if \lcode{USE_PATH_SPECIFIC_MODELS = .true.})
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
\lcode{spectral_kernel_PARAMETRIZATION_EVENTID_STATIONNAME} (in case of pre-integrated kernels on inversion grid)\\
\lcode{spectral_kernel_ON-WP_PARAMETRIZATION_EVENTID_STATIONNAME} (in case of plain kernel values on wavefield points)\\
For time kernels a similar format is used, just with \lcode{time_kernel_} instead of \lcode{spectral_kernel_}.
For generating vtk files, the above naming conventions are used as base names, extended by station component, 
model parameter, frequency (if applicable) as well as file extension \lcode{.vtk}
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
be named in the following way:\\
\lcode{synthetics_EVENTID_STATIONNAME_COMP}
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Text-based \lcodetitle{kim} File Format for Use with External Forward Codes} \label{files,sec:text-kim_files}
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%
Some executables (such as e.g.\ \lcode{exportKim}, \myref{programs_scripts,sec:bin_prog,sec:exp_Kim}
or \lcode{createStartmodelKim}, \myref{programs_scripts,sec:bin_prog,sec:create_startmod_kim})
export model descriptions on an inversion grid (e.g.\ given by a \lcode{.kim} file) to a text file
...
\end{itemize} 
where \lcode{c1,c2,c3} are the 3 coordinates of the cell center (in wavefield point coords, i.e.\ global 
Cartesian coordinates), \lcode{r} 
is the cell radius (i.e. rough expansion of cell), \lcode{nnb} is the number of cell neighbours and 
\lcode{inb1},...,\lcode{inbn} are their nnb cell indices (if \lcode{nnb}\(>\)\lcode{0}, otherwise line 
ends on \lcode{nnb}). Thereafter, \lcode{number_of_parameters} blocks follow, each having 
...
\end{itemize}
where \lcode{name_param} is the name of the parameter to which the following model values belong, 
\lcode{nval} is the number of model values following, \lcode{cell_indx} is a vector of \lcode{nval} 
cell indices to which the model values belong (space separated on one line) and \lcode{model_values} 
is a vector of the actual \lcode{nval} model values (space separated on one line).
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
